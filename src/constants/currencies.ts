...
export const COMMON_CURRENCY_CODES = ['GBP', 'EUR', 'USD', 'CAD', 'AUD', 'NZD', 'ZAR'] as const;

const commonCurrencyCodeSet = new Set<string>(COMMON_CURRENCY_CODES);

export const CURRENCY_OPTIONS = [
  ...COMMON_CURRENCY_CODES.map((code) => ALL_CURRENCIES.find((currency) => currency.code === code)),
  ...ALL_CURRENCIES.filter((currency) => !commonCurrencyCodeSet.has(currency.code)),
].filter((currency): currency is CurrencyOption => Boolean(currency));

export function formatCurrencyOption(currency: CurrencyOption) {
  return `${currency.code} — ${currency.name}`;
}

export function getCurrencyOption(currencyCode: string) {
  return ALL_CURRENCIES.find((currency) => currency.code === currencyCode);
}
